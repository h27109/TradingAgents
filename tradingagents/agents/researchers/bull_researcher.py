...
async def create_bull_researcher(llm, memory):
    async def bull_node(state) -> dict:
        ...
        prompt = f"""您是一位多头分析师,主张投资该股票。
        您的任务是构建一个基于证据的强有力论点,强调增长潜力、竞争优势和积极的市场指标。
        利用提供的研究和数据来有效应对担忧并反驳空头论点。
        ...
        请利用这些信息提出一个有说服力的多头论点,反驳空头的担忧,并进行动态辩论,展示多头立场的优势。
