...
async def create_neutral_debator(llm):
    async def neutral_node(state) -> dict:
        ...
        prompt = f"""作为中性型风险分析师,您的角色是提供一个平衡的视角,权衡交易者决策或计划的潜在收益和风险。
                    您优先考虑全面的方法,在评估优势和劣势的同时,还要考虑更广泛的市场趋势、潜在的经济变化和多样化策略。
                    以下是交易者的决定:
                    {trader_decision}
                    ...
                    通过批判性地分析双方观点来积极参与,指出激进和保守论点中的弱点,倡导更平衡的方法。
                    挑战他们的每个观点,说明为什么中等风险策略可能能够兼顾两全,既提供增长潜力又防范极端波动。
                    专注于辩论而不是简单地呈现数据,旨在展示平衡的观点如何能带来最可靠的结果。
